...
import { GitBranch, Clock, User, FileText, GripVertical, Plus, MoreHorizontal, AlertTriangle } from "lucide-react";
...
const statusColumns: Array<{ key: ColumnKey; label: string; dotColor: string; borderColor: string; bgColor: string }> = [
  { key: "open", label: "Open", dotColor: "bg-risk-watch", borderColor: "border-l-risk-watch", bgColor: "bg-risk-watch/5" },
  { key: "in_progress", label: "In Progress", dotColor: "bg-primary", borderColor: "border-l-primary", bgColor: "bg-primary/5" },
  { key: "pending_review", label: "Pending Review", dotColor: "bg-rbi", borderColor: "border-l-rbi", bgColor: "bg-rbi/5" },
  { key: "resolved", label: "Resolved", dotColor: "bg-risk-stable", borderColor: "border-l-risk-stable", bgColor: "bg-risk-stable/5" },
];
  ...
  return (
    ...
    >
      <Card
        ...
        className={`border-l-[3px] ${col.borderColor} border-border/60 bg-card/90 backdrop-blur-sm hover:bg-secondary/20 cursor-grab active:cursor-grabbing transition-all duration-200 rounded-xl group hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-0.5`}
      >
        <CardContent className="p-4">
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-start gap-2 flex-1 min-w-0">
              ...
              <p
                className="text-xs font-display font-medium text-foreground leading-snug group-hover:text-primary transition-colors cursor-pointer"
          ...
          {/* Vendor */}
          <p
            className="mt-2 text-[11px] text-primary font-display cursor-pointer hover:underline ml-6"
            onClick={(e) => { e.stopPropagation(); onNavigate(item.vendorId); }}
          >
            {item.vendorName}
          </p>
          ...
          {/* Meta */}
          <div className="mt-3 ml-6 space-y-1.5 text-[10px] text-muted-foreground">
            <div className="flex items-center gap-1.5">
              <User className="h-2.5 w-2.5" />
              <span className="font-display">{item.assignedTo}</span>
              <span className="text-muted-foreground/50">•</span>
              <span className="text-muted-foreground/70">{item.assignedRole}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Clock className={`h-2.5 w-2.5 ${isOverdue && colKey !== "resolved" ? "text-risk-high" : ""}`} />
              <span className={isOverdue && colKey !== "resolved" ? "text-risk-high font-semibold" : ""}>
...
export default function WorkflowsPage() {
  ...
  const handleDrop = useCallback((e: React.DragEvent, targetCol: ColumnKey) => {
    ...
    setItems(prev => prev.map(item =>
      item.id === itemId ? { ...item, status: targetCol } : item
    ));
    setDraggedId(null);
    setDragOverCol(null);
  }, []);
    ...
    <PageTransition className="space-y-6 p-5 lg:p-8 max-w-[1800px] mx-auto">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {criticalTasks > 0 && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-risk-high/5 border border-risk-high/20">
          ...
          {overdueTasks > 0 && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-risk-critical-foreground/5 border border-risk-critical-foreground/20">
              ...
              >
                ...
                <div className="mb-4 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="rounded-full bg-secondary/60 px-2 py-0.5 font-mono text-[10px] text-muted-foreground">{colItems.length}</span>
                  </div>
                  <button className="h-6 w-6 rounded-md hover:bg-secondary/50 flex items-center justify-center transition-colors">
                ...
                <div className="space-y-3">
                  <AnimatePresence mode="popLayout">
                    {colItems.map((item) => (
                      <KanbanCard
                        ...
                        onNavigate={(vendorId) => navigate(`/vendors/${vendorId}`)}
                      />
                    ))}
                  </AnimatePresence>
                  ...
                  {colItems.length === 0 && !isOver && (
                    <div className="rounded-xl border border-dashed border-border/40 py-10 text-center">
                      <p className="text-[11px] text-muted-foreground font-display">Drop items here</p>
                    </div>
                  )}
                  ...
                  {isOver && (
                    <motion.div
                      ...
                      className="rounded-xl border-2 border-dashed border-primary/30 bg-primary/5 flex items-center justify-center"
                    >
                      <p className="text-[11px] text-primary font-display font-medium">Drop here</p>
                    </motion.div>
                  )}
                </div>

                {/* Add Task Button */}
                <button className="mt-3 w-full flex items-center justify-center gap-1.5 rounded-xl border border-dashed border-border/40 py-2.5 text-[11px] text-muted-foreground font-display hover:border-primary/30 hover:text-primary hover:bg-primary/5 transition-all">
